...
`%APPDATA%\altgo\altgo.toml`（通常是 `C:\Users\<用户名>\AppData\Roaming\altgo\altgo.toml`）
...
与 [`configs/altgo.toml`](https://github.com/cislunarspace/altgo/blob/master/configs/altgo.toml) 一致；默认触发键为 **右 Alt** 的 keysym 名 `Alt_R`。若在设置页使用「按下以设置」，可能会写入 `linux_evdev_code` / `windows_vk` 等字段。
...
inject_at_cursor = true            # 尝试注入到光标位置（仅 Windows；失败时仍会走剪贴板/悬浮窗）
