...
**Linux**：通过子进程调用 CLI 工具（`parecord`、`xclip`、`xinput`），简化构建和依赖管理。

**Windows**：使用 Win32 API crate（`windows` 0.61）和跨平台 crate（`cpal` WASAPI、`arboard` 剪贴板）。键盘监听通过 `WH_KEYBOARD_LL` 全局钩子实现，音频通过 WASAPI 捕获。

每个平台模块通过 `#[cfg(target_os)]` 选择实现，暴露统一 trait（`KeyListener`、`Recorder`、`Output`），管道消费 `Box<dyn Trait>` 实现与平台解耦。

## 转写引擎

通过 `Transcriber` trait 分派（`async_trait`）：

- `WhisperApi` — HTTP multipart 上传到 OpenAI 兼容端点
- `MimoAsr` — 小米 MiMo-V2.5-ASR 云 API
- `SherpaTranscriber` — 内嵌 sherpa-onnx 的本地 SenseVoice，模型常驻内存
...
- 平台模块 — 仅构造/冒烟测试
- CI 在 Linux 上运行：`fmt`、`clippy`、`build --release`、`test`
