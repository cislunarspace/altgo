...
impl WindowsRecorder {
    ...
    > {
        ...
        let default = device
            ...
            .map_err(|e| RecorderError::CaptureFailed(format!("no supported input config: {e}")))?;
    ...
    pub fn start(&mut self) -> Result<(), RecorderError> {
        ...
        let device = host
            ...
            .ok_or_else(|| RecorderError::StartFailed("no input device available".to_string()))?;
        ...
        let stream: cpal::Stream = match sample_format {
            cpal::SampleFormat::F32 => {
                ...
                device
                    ...
                    .map_err(|e| {
                        RecorderError::CaptureFailed(format!("failed to build input stream: {e}"))
                    })?
            }
        ...
        stream.play().map_err(|e| {
            RecorderError::StartFailed(format!("failed to start input stream: {e}"))
        })?;
