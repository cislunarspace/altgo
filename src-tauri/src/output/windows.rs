...
//! 剪切板写入路径靠 Windows 手动验证（路线 B 选项 3）。
...
impl super::Output for WindowsOutput {
    fn write_clipboard(&self, text: &str) -> Result<(), OutputError> {
        ...
        let mut clipboard = arboard::Clipboard::new().map_err(|e| {
            OutputError::ClipboardFailed(format!("failed to access clipboard: {e}"))
        })?;
        clipboard
            .set_text(text.to_string())
            .map_err(|e| OutputError::ClipboardFailed(format!("failed to set clipboard text: {e}")))
    }
